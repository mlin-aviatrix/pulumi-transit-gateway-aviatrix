...
func construct(ctx *pulumi.Context, typ, name string, inputs provider.ConstructInputs,
	...
	switch typ {
	case "aviatrix:index:TransitModule":
		return constructTransitModule(ctx, name, inputs, options)
	default:
		return nil, errors.Errorf("unknown resource type %s", typ)
	}
}

func constructTransitModule(ctx *pulumi.Context, name string, inputs provider.ConstructInputs,
	options pulumi.ResourceOption) (*provider.ConstructResult, error) {
	...
	transitModule, err := NewTransitGateway(ctx, name, args, options)
	if err != nil {
		return nil, errors.Wrap(err, "creating component")
	}

	return provider.NewConstructResult(transitModule)
}
